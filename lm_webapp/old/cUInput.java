...
public class cUInput {
    ...
    static String mGetCtrlDescr(){      //171213
        ...
        if (mElement()!=null)
            if (cProtocol3.bShowAliases)
                s=oElemViewProps.mAlias() +" "+ oElemViewProps.mGetValueText();
            else
    ...
    private static void mSettingsFile_Load(Spinner cbSettingsFile, cOptionBox oListMemory) { //Load a file from factory,download or preferences
        ...
        if (oListMemory.mGetCheckedIndex()==0) {
            ...
            s = s+mFileRead(cProgram3.mContext,sFile_ProtCfg );
            mPref_Clear();            //Remove old preferences
            s=mStr2Pref(sFile_ProtCfg,s);                      //Transfer to preferences
            mPersistAllData(true,sFile_ProtCfg);  //Load setup
        } else {
    ...
    public void mZero() {
        if (oElemViewProps==null) return;       //Protection
        cProgram3.mCommandTxt("Zeroed");
    ...
    public void mChange(float nChange) {
        if (oElemViewProps==null) return;       //Protection
        oElemViewProps.mChangeValue( nChange);
        bDoRefresh=true;    //Makes the command button refresh
    }

    public void mDevice_Click() {
        if (cAndMeth.isEmpty(       cProgram3.sFile_ProtCfg)){
            mSelectFactoryFile();
        } else {
    ...
    private static String mGetText(Spinner inCB) {
        ...
        if (inCB.getSelectedItem()==null) {
            mErrMsg("null pointer 170623");return "";
        }
...
//            mLayoutAddTextEdit2("Refresh time (max: "+cProgram3.mLoopTime+")" , cProgram3.mLoopTime);
    ...
    switch (oProtocol().getState()) {       //Connection issues
        case kProtReady:
            if (mElement()!=null) {
        ...
        case kProtInitDone:
                break;
        case kProtInitFailure:  //One of the following conditions
        case kUnconnected2:
        case kProtUndef1:
        case kProtTimeOut:
        case kProtError:
    ...
    private static void mReConnect() {       //Connect if not connected
        for (int i = 0; i< oaProtocols.length; i++)
           oaProtocols[i].mStateSet(kDoReconnect);
    }
...
/*!-    cleaning 170919        if (myEditControl instanceof cVertSlider2) {
    ...
    public void mSelectFactoryFile(){
        mDialogGridInit1("Select a Setup File");          //Set dialog boxes
        String[] s = mGetRawFileNames();
        cbFactorySettingsFile= mAddCombo("File", s,sFile_ProtCfg);
        alert.setPositiveButton( "Load", new DialogInterface.OnClickListener() {     //ToDo 180413: Improve this layout
            public void onClick(DialogInterface dialog, int id) {
                cProgram3.mLoadFactorySettings(mGetText(cbFactorySettingsFile));
                cbSettingsFile=null;
                mClose1();
            }});
        mAlertShow();
    ...
    void mEditControl2_Accept(){
        ...
        if (oCB2 != null) mFocusedViewProps().bEnabled( oCB2.isChecked());        oCB2=null;
        if (oCB1 != null) mFocusedViewProps().bVisible( oCB1.isChecked());        oCB1=null;
        bDoRedraw=true;
    };
