...
//        s = s.replace("[", "").replace("]", "");
    ...
    public static int mStr2Int(String text) {
        ...
        try {
            if (text.contains("e")||text.contains("E")){ //Scientific notation (bugfix180808)
                retval= (int) Double.parseDouble(text);     //Thanks to https://stackoverflow.com/questions/2546147/java-convert-scientific-notation-to-regular-int
                return  retval;
            }
            retval= Integer.parseInt(text);
            return retval;
        } catch (NumberFormatException e) {     //Try to decode hexadecimal hex2int
            if (text.contains("0X")) text=text.replaceFirst("[0][xX]","");
