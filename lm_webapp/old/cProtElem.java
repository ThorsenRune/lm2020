...
//-161216 obsolete      private  int nVarCount;	//The size of the protocol i.e. the number of variables exposed  nVarCount
    ...
    public void mSettings1(boolean bGet,String sMyKey){        //mFileSave the
        ...
        if (mIsBitField()||(bGet)) {        //Do only save real bitfields
            ...
            nBitVisible = mPrefs5(bGet, sMyKey + ".BitDisp", nBitVisible);      //Todo: Todo1 make Obsolete
        }
    }
    ...
    public void setVal(int idx, float val) {        //Set a value in the relative units
        ...
        if (nDisplayRange[0]<nDisplayRange[1]) {     //Range control enabled
            if (val < nDisplayRange[0]) {
                val = nDisplayRange[0];
                mMsgLog(10,"Clipping value to " + val);
...
//        aData[idx]=(int) (val* nFactor+0.5)+nOffset;    //Convert units to integer, rounding to nearest integer
    ...
    public void mChangeValue(int elemDataIdx, float nChange) {//Change the value with a fraction of the scale
        if (aData.length <= elemDataIdx) mErrMsg("Index fault");
        //Clip value to display range
        float nOldVal = getVal((elemDataIdx));
        float nNewVal = nOldVal +(nDisplayRange[1] - nDisplayRange[0]) * nChange;
        setVal(elemDataIdx,nNewVal);
    }
...
//        return nDataArraySize;
    ...
    public int mBitVisible(int i) {
        ...
        if (sBitNames.length<=i)        //Too small array
            return 0;
        if (sBitNames[i].trim().length()<1)
            nBitVisible[i]=0;                   //Hide bits with no names
        else
            nBitVisible[i]=1;                   //Show if it has a name
